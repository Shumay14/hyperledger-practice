import {
  ...
  $ as $$,
  e as assertNotReached,
  f as strictQuery,
  F as FocusOutlineManager,
  r as recordVoiceAction,
  A as Action,
  g as recordLoadDuration,
  i as hexColorToSkColor,
  B as BrowserCommandProxy,
  C as CustomizeDialogPage,
} from "./shared.rollup.js";
export {
  $ as $$,
  B as BrowserCommandProxy,
  j as CrAutoImgElement,
  C as CustomizeDialogPage,
  I as IframeElement,
  N as NewTabPageProxy,
  A as VoiceAction,
  p as VoiceError,
  W as WindowProxy,
  o as createScrollBorders,
  d as decodeString16,
  c as mojoString16,
  k as recordDuration,
  g as recordLoadDuration,
  l as recordOccurence,
  n as recordPerdecage,
} from "./shared.rollup.js";
import {
  html,
  PolymerElement,
...
import {
  PageHandler,
  PageCallbackRouter,
} from "chrome://new-tab-page/realbox.mojom-webui.js";
import {
  DoodleShareChannel,
  DoodleImageType,
} from "chrome://new-tab-page/new_tab_page.mojom-webui.js";
...
function getTemplate$7() {
  return html`<!--_html_template_start_--><style>
      ...
      :host-context(ntp-realbox-match[is-rich-suggestion][has-image])
        #container {
        background-color: var(--container-bg-color);
      }

      /* Calculator answer and suggestion answer icons feature a blue background. */
      :host-context(ntp-realbox-match[is-rich-suggestion]:not([has-image]))
        #container {
      ...
      :host([in-searchbox][background-image="realbox/icons/google_g.svg"])
        #icon {
...
class RealboxIconElement extends PolymerElement {
  ...
  computeImageSrc_() {
    ...
    if (this.match.imageDataUrl) {
      ...
      this.match.imageUrl &&
      this.match.imageUrl.startsWith("data:image/")
    ) {
  ...
  containerBgColor_(imageSrc, imageDominantColor) {
    return !imageSrc && imageDominantColor ? `${imageDominantColor}40` : "";
  }
}
  ...
  return html`<!--_html_template_start_--><style import="cr-shared-style">
      ...
      :host(:hover) {
        background-color: var(
          --action-bg-hovered,
          rgba(var(--google-grey-900-rgb), 0.1)
        );
      }
...
customElements.define(RealboxActionElement.is, RealboxActionElement); // Copyright 2020 The Chromium Authors. All rights reserved.
let instance$1 = null;
class RealboxBrowserProxy {
  ...
  static getInstance() {
    return instance$1 || (instance$1 = new RealboxBrowserProxy());
  }
...
function getTemplate$5() {
  return html`<!--_html_template_start_--><style
    ...
    >
      ...
      :host(:is(:focus-visible, .selected:not(:focus-within)))
        #focus-indicator {
        display: block;
      }
      ...
      :host-context(ntp-realbox-match:-webkit-any(:focus-within, .selected)):host([is-rich-suggestion])
        #description,
      :host-context(ntp-realbox-match:-webkit-any(:focus-within, .selected))
        .dim {
      ...
      #remove {
        --cr-icon-button-active-background-color: rgba(
          var(--google-grey-900-rgb),
          0.16
        );
        ...
        --cr-icon-button-hover-background-color: rgba(
          var(--google-grey-900-rgb),
          0.1
        );
    ...
    <div class="container" aria-hidden="true">
      ...
      <cr-icon-button
        ...
        on-mousedown="onRemoveButtonMouseDown_"
        title="[[removeButtonTitle_]]"
        hidden$="[[!match.supportsDeletion]]"
        tabindex="2"
      >
      </cr-icon-button>
    </div>
...
(function (ACMatchClassificationStyle) {
  ...
  ACMatchClassificationStyle[(ACMatchClassificationStyle["MATCH"] = 2)] =
    "MATCH";
...
class RealboxMatchElement extends PolymerElement {
  ...
  computeContentsHtml_() {
    ...
    const matchContents = match.answer
      ? match.answer.firstLine
      : match.contents;
    return match.swapContentsAndDescription
      ? this.renderTextWithClassifications_(
          decodeString16(match.description),
          match.descriptionClass
        ).innerHTML
      : this.renderTextWithClassifications_(
          decodeString16(matchContents),
          match.contentsClass
        ).innerHTML;
  }
  ...
  renderTextWithClassifications_(text, classifications) {
    return classifications
      .map(({ offset: offset, style: style }, index) => {
...
function getTemplate$4() {
  return html`<!--_html_template_start_--><style>
    ...
    <iron-selector
      id="selector"
      selectable="ntp-realbox-match"
      items="{{selectableMatchElements_}}"
      selected="{{selectedMatchIndex}}"
      ...
      <template is="dom-repeat" id="groups" items="[[groupIds_]]" as="groupId">
        <template is="dom-if" if="[[groupHasHeader_(groupId)]]">
          ...
          <div
            ...
            data-id$="[[groupId]]"
            tabindex="-1"
            on-focusin="onHeaderFocusin_"
            on-click="onHeaderClick_"
            aria-hidden="true"
        ...
        <template
          is="dom-if"
          if="[[!groupIsHidden_(groupId, hiddenGroupIds_.*)]]"
          restamp=""
        >
          <template
            is="dom-repeat"
        ...
        <template> <!--_html_template_end_--></template></template
      ></iron-selector
    >`;
} // Copyright 2020 The Chromium Authors. All rights reserved.
class RealboxDropdownElement extends PolymerElement {
  ...
  static get properties() {
    return {
      ...
      theme: { type: Object, observer: "onThemeChange_" },
      groupIds_: { type: Array, computed: `computeGroupIds_(result)` },
      hiddenGroupIds_: {
        type: Array,
  ...
  onResultRepaint_() {
    this.dispatchEvent(
      new CustomEvent("result-repaint", {
        bubbles: true,
        composed: true,
  ...
  onThemeChange_() {
    ...
    this.updateStyles({
      "--search-box-icon-selected": skColorToRgba(
        assert(this.theme.iconSelected)
      ),
      "--search-box-icon": skColorToRgba(assert(this.theme.icon)),
      "--search-box-results-bg-hovered": skColorToRgba(
        assert(this.theme.resultsBgHovered)
      ),
      "--search-box-results-bg": skColorToRgba(assert(this.theme.resultsBg)),
      "--search-box-results-dim-selected": skColorToRgba(
        assert(this.theme.resultsDimSelected)
      ),
      "--search-box-results-dim": skColorToRgba(assert(this.theme.resultsDim)),
      "--search-box-results-text": skColorToRgba(
        assert(this.theme.resultsText)
      ),
      "--search-box-results-url-selected": skColorToRgba(
        assert(this.theme.resultsUrlSelected)
      ),
  ...
  onHeaderFocusin_() {
    this.dispatchEvent(
      new CustomEvent("header-focusin", { bubbles: true, composed: true })
    );
  }
  onHeaderClick_(e) {
  ...
  computeGroupIds_() {
    ...
    return [
      ...new Set(this.result.matches.map((match) => match.suggestionGroupId)),
    ];
  }
...
function getTemplate$3() {
  return html`<!--_html_template_start_--><style>
      :host {
        ...
        --ntp-realbox-shadow: 0 1px 6px 0 rgba(32, 33, 36, 0.28);
    ...
    <div
      id="inputWrapper"
      on-focusout="onInputWrapperFocusout_"
    ...
    >
      <input
        ...
        autocomplete="off"
        spellcheck="false"
        aria-live="[[inputAriaLive_]]"
        role="combobox"
        placeholder="Search Google or type a URL"
        on-copy="onInputCutCopy_"
        on-cut="onInputCutCopy_"
        on-focus="onInputFocus_"
        on-blur="onInputBlur_"
        on-input="onInputInput_"
        on-keydown="onInputKeydown_"
        on-keyup="onInputKeyup_"
        on-mousedown="onInputMouseDown_"
        on-paste="onInputPaste_"
      />
      <ntp-realbox-icon
        id="icon"
        match="[[selectedMatch_]]"
        default-icon="[[realboxIcon_]]"
        in-searchbox=""
      >
      </ntp-realbox-icon>
      <button
      ...
      <ntp-realbox-dropdown
        ...
        on-match-remove="onMatchRemove_"
...
class RealboxElement extends PolymerElement {
  ...
  static get properties() {
    return {
      ...
      charTypedTime_: { type: Number, value: 0 },
      ...
      lastInputFocusTime_: { type: Number, value: null },
  ...
  onThemeChange_() {
    ...
    this.updateStyles({
      "--search-box-border-color": skColorToRgba(
        assert(this.theme.borderColor)
      ),
  ...
  onInputCutCopy_(e) {
    ...
    if (this.selectedMatch_ && !this.selectedMatch_.isSearchType) {
      e.clipboardData.setData(
        "text/plain",
        this.selectedMatch_.destinationUrl.url
      );
      e.preventDefault();
      if (e.type === "cut") {
        this.updateInput_({ text: "", inline: "" });
  ...
  onInputInput_(e) {
    ...
    this.charTypedTime_ = charTyped
      ? this.charTypedTime_ || window.performance.now()
      : 0;
    if (inputValue.trim()) {
  ...
  onInputKeydown_(e) {
    ...
    const lastInputValue = this.lastInput_.text + this.lastInput_.inline;
    if (
      inputSelection === this.lastInput_.inline &&
  ...
  navigateToMatch_(matchIndex, e) {
    ...
    const delta = mojoTimeDelta(
      window.performance.now() - this.lastInputFocusTime_
    );
    this.pageHandler_.openAutocompleteMatch(
      matchIndex,
      match.destinationUrl,
      this.matchesAreVisible,
      delta,
      e.button || 0,
      e.altKey,
      e.ctrlKey,
  ...
  updateInput_(update) {
    ...
    if (newInputValue.trim() && needsSelectionUpdate) {
      ...
      this.$.input.selectionEnd = preserveSelection
        ? oldSelectionEnd
        : newInputValue.length;
    }
    this.isDeletingInput_ =
...
function getTemplate$2() {
  return html`<!--_html_template_start_--><style>
    ...
    <cr-dialog id="dialog" show-on-attach="">
      <div id="title" slot="title">[[title]]</div>
      <div slot="body">
        <div id="buttons">
          <cr-button
            id="facebookButton"
...
class DoodleShareDialogElement extends PolymerElement {
  ...
  onEmailClick_() {
    const url =
      `mailto:?subject=${encodeURIComponent(this.title)}` +
...
class LogoElement extends PolymerElement {
  ...
  onImageDoodleChange_() {
    ...
    if (shareButton) {
      ...
      this.updateStyles({
        "--ntp-logo-share-button-background-color": skColorToRgba(
    ...
    if (this.imageDoodle_) {
      this.updateStyles({
        "--ntp-logo-box-color": skColorToRgba(
          this.imageDoodle_.backgroundColor
        ),
      });
    } else {
  ...
  onImageLoad_() {
    this.logImageRendered_(
      this.isCtaImageShown_() ? DoodleImageType.kCta : DoodleImageType.kStatic,
  ...
  onShare_(e) {
    const doodleId = new URL(
      this.doodle_.image.onClickUrl.url
    ).searchParams.get("ct");
    if (!doodleId) {
      return;
    }
...
function getTemplate() {
  return html`<!--_html_template_start_--><style include="cr-shared-style">
      ...
      :host([show-background-image_]) {
        --ntp-theme-text-shadow: 0 0 16px rgba(0, 0, 0, 0.3);
      }
     ...
     and ntp-modules. */
      ...
      #backgroundImageAttribution:hover {
        background: rgba(var(--google-grey-900-rgb), 0.1);
      }
        ...
        <template>
          ...
          >
            <div id="backgroundImageAttribution1Container">
              <div
                id="linkIcon"
                hidden="[[!backgroundImageAttributionUrl_]]"
              ></div>
            ...
            <div
              id="backgroundImageAttribution2"
    ...
    <template
      id="customizeDialogIf"
      is="dom-if"
      if="[[showCustomizeDialog_]]"
      restamp=""
    >
...
class LoadTimeResolver {
  constructor(url) {
    ...
    this.eventTracker_.add(window, "message", ({ data: data }) => {
      if (
        data.frameType === "background-image" &&
...
class BackgroundManager {
  constructor() {
    ...
    this.backgroundImage_ = strictQuery(
      document.body,
      "#backgroundImage",
      HTMLIFrameElement
    );
...
class AppElement extends PolymerElement {
  constructor() {
    ...
    this.shouldPrintPerformance_ = new URLSearchParams(location.search).has(
      "print_perf"
    );
  ...
  connectedCallback() {
    super.connectedCallback();
    this.setThemeListenerId_ = this.callbackRouter_.setTheme.addListener(
      (theme) => {
      ...
      this.backgroundManager_.getBackgroundImageLoadTime().then(
        (time) => {
          ...
          this.printPerformanceDatum_(
            ...
            this.backgroundImageLoadStart_,
            duration
          );
          this.printPerformanceDatum_(
  ...
  computeRealboxShown_() {
    return (
      !loadTimeData.getBoolean("realboxMatchOmniboxTheme") || !!this.theme_
    );
  }
  ...
  executePromoBrowserCommand_(commandData, commandSource, commandOrigin) {
    ...
    BrowserCommandProxy.getInstance()
      ...
      .then(({ commandExecuted: commandExecuted }) => {
  ...
  printPerformance_() {
    ...
    const log = (entry) => {
      this.printPerformanceDatum_(
        entry.name,
        entry.duration ? entry.duration : entry.startTime,
